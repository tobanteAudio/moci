...
TEST_CASE("moci/core: PropertyMapKeyTypes", "[core]")
{
    {
...
TEST_CASE("moci/core: PropertyMapDefaultConstruct", "[core]")
{
    auto const tree = moci::PropertyMap<std::uint32_t>();
    REQUIRE(tree.GetName() == "");
}

TEST_CASE("moci/core: PropertyMapNamedConstruct", "[core]")
{
    auto const tree = moci::PropertyMap<std::uint32_t>("test");
    REQUIRE(tree.GetName() == "test");
}

TEST_CASE("moci/core: PropertyMapSetName", "[core]")
{
...
TEST_CASE("moci/core: PropertyMapGetProperty", "[core]")
{
    auto tree = moci::PropertyMap<std::uint32_t>();
...
TEST_CASE("moci/core: PropertyMapSetProperty", "[core]")
{
    auto tree = moci::PropertyMap<std::uint32_t>();
    tree.SetProperty(143, "test");
    auto const p = tree.GetProperty(143).value();
...
TEST_CASE("moci/core: PropertyMapDeleteProperty", "[core]")
{
    auto tree = moci::PropertyMap<std::uint32_t>();
...
TEST_CASE("moci/core: PropertyMapHasProperty", "[core]")
{
    auto tree = moci::PropertyMap<std::uint32_t>();
    REQUIRE(tree.HasKey(143) == false);
    tree.SetProperty(143, "test");
