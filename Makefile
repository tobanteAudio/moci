...
.PHONY: config
config:
	cmake -G$(CMAKE_GENERATOR) $(CMAKE_FLAGS) -B$(BUILD_DIR_BASE)_$(CONFIG) -DCMAKE_BUILD_TYPE=$(CONFIG) -DMOCI_BUILD_TESTS=ON .

.PHONY: config-gles
config-gles:
	cmake -G$(CMAKE_GENERATOR) $(CMAKE_FLAGS) -B$(BUILD_DIR_BASE)_$(CONFIG) -DCMAKE_BUILD_TYPE=$(CONFIG) -DMOCI_GRAPHICS_API_GLES=ON .
...
.PHONY: stats
stats:
	@cloc --exclude-dir=3rd_party,build_Debug,build_Release,venv .
