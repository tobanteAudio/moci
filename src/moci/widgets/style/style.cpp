...
void Style::drawButton(Painter& painter, Rectangle<int> bounds, Button const& button)
{
    ...
    painter.drawText(
        button.getText(),
        {bounds.getX() + width / 2, bounds.getY() + height},
        0.5F,
        color
    );
}
...
void Style::drawLabel(Painter& painter, Rectangle<int> bounds, Label const& label)
{
    auto area = bounds;
    painter
        .drawText(label.getText(), {area.getX(), area.getY()}, 0.5F, label.getTextColor());
}
...
void Style::drawSlider(Painter& painter, Rectangle<int> bounds, Slider const& slider)
{
    ...
    // infill
    auto const inFillWidth = static_cast<int>(
        static_cast<float>(width - border * 4) * std::min(slider.getValue(), 1.0F)
    );
