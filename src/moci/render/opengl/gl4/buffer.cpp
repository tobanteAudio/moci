...
auto OpenGLVertexBuffer::uploadData(
    std::uint32_t offset,
    std::uint32_t size,
    void const* data
) const -> void
...
OpenGLIndexBuffer::OpenGLIndexBuffer(IndexBufferSpecs specs) : _specs(specs)
{
    ...
    if (_specs.isDynamic) {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, size, _specs.indices.data(), GL_STATIC_DRAW);
    }
...
auto OpenGLIndexBuffer::uploadData(std::uint32_t offset, std::span<std::uint32_t> indices)
    const -> void
{
