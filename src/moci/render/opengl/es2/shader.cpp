...
auto getAttributeLayout(std::string const& src) -> std::vector<ShaderAttribute>
{
    ...
    while (std::getline(f, line)) {
        ...
        if (found != std::string::npos) {
            ...
            auto type = splits[1];
            auto name = splits[2];
            name.erase(std::find_if(name.begin(), name.end(), [](auto ch) {
                return ch == ';';
            }));
...
OpenGLESShader::OpenGLESShader(
    ...
    : _rendererID(createShader(vertexSrc.c_str(), fragmentSrc.c_str()))
    , _name(std::move(name))
{}
...
auto OpenGLESShader::createShader(char const* vertexSource, char const* fragmentSource)
    -> GLint
{
...
void OpenGLESShader::setFloat2(std::string const& name, glm::vec2 const& value)
{
    uploadUniformFloat2(name, value);
}

void OpenGLESShader::setFloat3(std::string const& name, glm::vec3 const& value)
{
    uploadUniformFloat3(name, value);
}

void OpenGLESShader::setFloat4(std::string const& name, glm::vec4 const& value)
{
    uploadUniformFloat4(name, value);
}

void OpenGLESShader::setMat3(std::string const& name, glm::mat3 const& value)
{
    uploadUniformMat3(name, value);
}

void OpenGLESShader::setMat4(std::string const& name, glm::mat4 const& value)
{
...
void OpenGLESShader::uploadUniformFloat2(std::string const& name, glm::vec2 const& value)
    const
{
    GLCall(glUniform2f(getLocation(name), value.x, value.y));
}

void OpenGLESShader::uploadUniformFloat3(std::string const& name, glm::vec3 const& value)
    const
{
    GLCall(glUniform3f(getLocation(name), value.x, value.y, value.z));
}

void OpenGLESShader::uploadUniformFloat4(std::string const& name, glm::vec4 const& value)
    const
{
    GLCall(glUniform4f(getLocation(name), value.x, value.y, value.z, value.w));
}

void OpenGLESShader::uploadUniformMat3(std::string const& name, glm::mat3 const& matrix)
    const
{
    GLCall(glUniformMatrix3fv(getLocation(name), 1, GL_FALSE, glm::value_ptr(matrix)));
}

void OpenGLESShader::uploadUniformMat4(std::string const& name, glm::mat4 const& matrix)
    const
{
