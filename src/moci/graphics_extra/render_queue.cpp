...
BatchRender2D::BatchRender2D()
{
    ...
    _data.vbo.reset(moci::RenderFactory::makeVertexBuffer(
        nullptr,
        sizeof(Vertex) * maxVertexCount,
        true
    ));
...
auto BatchRender2D::endBatch() -> void
{
    ...
    auto const verticesSize
        = static_cast<std::uint32_t>(_data.vertices.size() * sizeof(Vertex));
...
{
    MOCI_PROFILE_FUNCTION();
    for (size_t i = 0; i < _data.textures.size(); i++) {
        _data.textures[i]->bind(static_cast<std::uint32_t>(i));
    }
    ...
    _data.vao->unbind();
    for (auto const& tex : _data.textures) {
        tex->unbind();
    }
...
auto BatchRender2D::drawQuad(
    Rectangle<float> rect,
    ColorRGBA32 color,
    Texture2D::Optional texture
) -> void
{
    ...
    for (auto const i : {0, 1, 2, 2, 3, 0}) {
        _data.indices.push_back(static_cast<uint32_t>(_data.indexOffset + i));
    }
    _data.indexOffset += 4;
...
{
    flushIf(
        _data.indices.size() + (static_cast<size_t>(3) * static_cast<size_t>(numSides))
        >= maxIndexCount
    );
    ...
    for (auto i = 1; i < numVertices; i++) {
        auto const newX
            = x + (radius * glm::cos(i * doublePI / static_cast<float>(numSides)));
        auto const newY
            = y + (radius * glm::sin(i * doublePI / static_cast<float>(numSides)));
        _data.vertices.push_back(Vertex{
            {newX, newY, 0.0F},
            color,
            {0.0F, 0.0F},
...
auto BatchRender2D::getFrameStats() const -> BatchRender2D::FrameStats
{
    return _data.renderFrameStats;
}
