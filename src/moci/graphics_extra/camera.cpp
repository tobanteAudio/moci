...
void OrthographicCameraController::onUpdate(Timestep ts)
{
    ...
    if (_rotation)
    {
        if (Input::isKeyPressed(moci::Key::Q)) { _cameraRotation += _cameraRotationSpeed * ts; }
        if (Input::isKeyPressed(moci::Key::E)) { _cameraRotation -= _cameraRotationSpeed * ts; }

        if (_cameraRotation > 180.0F) { _cameraRotation -= 360.0F; }
        else if (_cameraRotation <= -180.0F) { _cameraRotation += 360.0F; }
