...
void DemoLayer::onAttach()
{
...

    {
        ...
        for (auto const& vertex : _mesh.getVertices())
        {
            ...
            auto const scaleMatrix = glm::scale(glm::mat4(1.0F), {_modelScale, _modelScale, _modelScale});
            auto const position    = model * scaleMatrix * glm::vec4(vertex.position, 1.0F);
            _vertices.push_back({glm::vec3(position), vertex.normal, vertex.color, vertex.texCoord});
        }
        ...
        for (auto const& vertex : _floor.getVertices())
        {
            ...
            auto const position    = model * scaleMatrix * glm::vec4(vertex.position, 1.0F);
            _vertices.push_back({glm::vec3(position), vertex.normal, {1.0F, 1.0F, 0.5F, 1.0F}, vertex.texCoord});
        }
    ...
    auto const size = static_cast<std::uint32_t>(_vertices.size() * sizeof(moci::Mesh::Vertex));
    ...
    _textureColors = moci::RenderFactory::makeTexture2D("src/app/sandbox3D/assets/textures/cerberus_A_4096x4096.png");
...
void DemoLayer::onUpdate(moci::Timestep ts)
{
    ...
    auto const projection  = glm::perspective(glm::radians(_cameraFOV), aspectRatio, 0.1F, 100.0F);
...
bool DemoLayer::onKeyPressed(moci::KeyPressedEvent& e)
{
    ...
    if (e.getKeyCode() == moci::Key::Up) { _cameraPos += cameraSpeed * _cameraFront; }
...
bool DemoLayer::onMouseScrolled(moci::MouseScrolledEvent& e)
{
    ...
    if (_cameraFOV < minFOV) { _cameraFOV = minFOV; }
    if (_cameraFOV > maxFOV) { _cameraFOV = maxFOV; }
    return true;
}
...
void DemoLayer::onImGuiRender()
{
    ...
    if (frameCount >= 100)
    {
        if (fps < _drawStats.minFPS) { _drawStats.minFPS = fps; }
        if (fps > _drawStats.maxFPS) { _drawStats.maxFPS = fps; }
    }
    ...
    auto* const textureID = reinterpret_cast<void*>((size_t)_framebuffer->getColorAttachmentRendererId());
    ...
    if (_imguiWindow)
    {
        ...
        if (ImGui::CollapsingHeader("Light"))
        {
            ...
            ImGui::SliderFloat3("Light Position", glm::value_ptr(_light->position), -20.0F, 20.0F);
        ...
        if (ImGui::CollapsingHeader("Stats"))
        {
            ...
            auto const mb           = _drawStats.numVertices * sizeof(moci::Mesh::Vertex) / 1'000'000.0F;
            ...
            ImGui::PlotLines("FPS", _fpsHistory.data(), size, 0, "", 50.0F, 75.0F, ImVec2(0, 80));
        }
        ImGui::End();
    }

    // ImGui Demo
    if (_imguiDemo) { ImGui::ShowDemoWindow(&_imguiDemo); }
}
